...
that due to a glaring misuse of some _Node.js_ APIs, all URLs have been missing
...
[^1]:
    For the curious, I inadvertently used the `path.join(…)` method instead of
    `url.resolve(…)` to construct these URLs. The former is only supposed to be
    used in the context of file system paths, _not_ URLs.
